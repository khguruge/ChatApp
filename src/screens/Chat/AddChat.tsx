      ...
      .then((snapshot) => {
        ...
        ref
          ...
          .then((snapshot) => {
            ...
            const temp = data.friends
              ? [...data.friends, user?.uid]
              : [user?.uid];
            const chat = data.chats
              ? [...data.chats, chatId.key]
              : [chatId.key];
            data.friends = temp;
            data.chats = chat;
            ref
              ...
              .then(() => {
                ref
                  .ref("/User/" + user?.uid + "/")
                  .update({ friends: tempFriends, chats: chats })
                  .then(() => {
                    loadData();
                    setLoading(false);
                  })
      ...
      <Content>
        <List style={{ flex: 1 }}>
          {userList.length === 0 ? (
            ...
            >
              <Text style={{ color: Color.NAVYBLUE, fontWeight: "bold" }}>
                No Users Available!
              </Text>
            </View>
          ) : (
